...
const HEARING_RANGE_SPRINT := 10.0
...
func _on_hearing_body_entered(body: Node) -> void:
	if body.is_in_group("player"):
		var p = body
		var is_crouching = p.get("is_crouching") if p.get("is_crouching") != null else false
		var is_prone = p.get("is_prone") if p.get("is_prone") != null else false
